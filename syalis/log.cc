...
void LogFormatter::init() {
    ...
    for(size_t i = 0; i < m_pattern.size(); ++i) {
        ...
        if(fmt_status == 0) {
            ...
            std::cout << "pattern parse error: " << m_pattern << " - " << m_pattern.substr(i) << std::endl; // 调试
...
#undef XX
   ...
   for(auto& item : vec) {
        ...
        // std::cout << "(" << std::get<0>(item) << ") - (" << std::get<1>(item) << ") - (" << std::get<2>(item) << ")" << std::endl; // 调试
   }
   // std::cout << m_items.size() << std::endl; // 调试
}
